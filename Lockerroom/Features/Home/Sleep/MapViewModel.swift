...
class MapViewModel: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    // --- FIX: Renamed to match SleepViewModel ---
    @Published var selectedDestination: MKMapItem?
    @Published var expectedCommuteTime: TimeInterval = 0 // In seconds
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        if let location = locations.first {
            userLocation = location
            manager.stopUpdatingLocation() // Stop updating to save power
        }
    }
    ...
    /// This function now matches what SleepViewModel expects.
    func calculateCommuteTime() {
        ...
        guard let destination = selectedDestination else {
            travelTimeMessage = "No destination selected."
            self.expectedCommuteTime = 0
            self.travelTimeMessage = ""
            self.destinationName = ""
            return
        }
        
        self.travelTimeMessage = "Calculating..."
        self.destinationName = destination.name ?? "Selected Location"
        ...
        directions.calculate { [weak self] response, error in
            ...
            self.expectedCommuteTime = route.expectedTravelTime
